...
import {
  DISCUSSION_MUSIC_DEFAULT_VOLUME,
  DISCUSSION_MUSIC_MAX_VOLUME,
  LOBBY_MUSIC_DEFAULT_VOLUME,
  LOBBY_MUSIC_MAX_VOLUME,
  setDiscussionMusicVolume as applyDiscussionMusicVolume,
  setLobbyMusicVolume as applyLobbyMusicVolume,
...
const APP_CONFIG_API_URL =
  (Constants.expoConfig?.extra as { apiUrl?: string } | undefined)?.apiUrl;
const NATIVE_PROD_FALLBACK_SOCKET_URL = "https://api.davetsizmisafir.app";
...
const SOCKET_URL = EXPLICIT_SOCKET_URL
  ...
  : DOMAIN
    ...
    : Platform.OS === "web"
      ...
      : NATIVE_PROD_FALLBACK_SOCKET_URL;
...
interface GameCtx {
  ...
  voiceMuted: boolean;
  toggleVoice: () => void;
  lobbyMusicEnabled: boolean;
  toggleLobbyMusic: () => void;
  lobbyMusicVolume: number;
  setLobbyMusicVolume: (value: number) => void;
  discussionMusicEnabled: boolean;
  toggleDiscussionMusic: () => void;
  discussionMusicVolume: number;
  setDiscussionMusicVolume: (value: number) => void;
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const [lobbyMusicEnabled, setLobbyMusicEnabledState] = useState(true);
  const [lobbyMusicVolume, setLobbyMusicVolumeState] = useState(LOBBY_MUSIC_DEFAULT_VOLUME);
  const [discussionMusicEnabled, setDiscussionMusicEnabledState] = useState(true);
  const [discussionMusicVolume, setDiscussionMusicVolumeState] = useState(DISCUSSION_MUSIC_DEFAULT_VOLUME);
  ...
  useEffect(() => {
    Promise.all([
      ...
      AsyncStorage.getItem("mahalle:lobbyMusicEnabled"),
      AsyncStorage.getItem("mahalle:lobbyMusicVolume"),
      AsyncStorage.getItem("mahalle:discussionMusicEnabled"),
      AsyncStorage.getItem("mahalle:discussionMusicVolume"),
      ...
      .then(([
        ...
        const mergedEnabled = (lobbyEnabled ?? discussionEnabled ?? legacyEnabled) !== "false";
        setLobbyMusicEnabledState(lobbyEnabled !== null ? lobbyEnabled !== "false" : mergedEnabled);
        setDiscussionMusicEnabledState(
          discussionEnabled !== null ? discussionEnabled !== "false" : mergedEnabled,
        );

        const legacyParsedVolume = Number(legacyVolume);
        const parseMusicVolume = (rawValue: string | null, maxVolume: number, fallbackDefault: number) => {
          const candidate = rawValue == null ? legacyParsedVolume : Number(rawValue);
          if (!Number.isFinite(candidate)) return fallbackDefault;
          const normalized = candidate > maxVolume ? fallbackDefault : candidate;
          return Math.max(0, Math.min(maxVolume, normalized));
        };

        const nextLobbyVolume = parseMusicVolume(lobbyVolume, LOBBY_MUSIC_MAX_VOLUME, LOBBY_MUSIC_DEFAULT_VOLUME);
        const nextDiscussionVolume = parseMusicVolume(
          discussionVolume,
          DISCUSSION_MUSIC_MAX_VOLUME,
          DISCUSSION_MUSIC_DEFAULT_VOLUME,
        );
        setLobbyMusicVolumeState(nextLobbyVolume);
        setDiscussionMusicVolumeState(nextDiscussionVolume);
        applyLobbyMusicVolume(nextLobbyVolume);
        applyDiscussionMusicVolume(nextDiscussionVolume);
        AsyncStorage.setItem("mahalle:lobbyMusicVolume", String(nextLobbyVolume));
        AsyncStorage.setItem("mahalle:discussionMusicVolume", String(nextDiscussionVolume));
        AsyncStorage.setItem("mahalle:lobbyMusicEnabled", String(lobbyEnabled !== null ? lobbyEnabled !== "false" : mergedEnabled));
        AsyncStorage.setItem(
          "mahalle:discussionMusicEnabled",
          String(discussionEnabled !== null ? discussionEnabled !== "false" : mergedEnabled),
        );
  ...
  const toggleLobbyMusic = useCallback(() => {
    setLobbyMusicEnabledState((prev) => {
      const next = !prev;
      AsyncStorage.setItem("mahalle:lobbyMusicEnabled", String(next));
      if (isMidGame()) {
        setSystemToast({
          message: next ? "Ana Tema Müziği açıldı" : "Ana Tema Müziği kapatıldı",
          id: Date.now(),
  ...
  const setLobbyMusicVolume = useCallback((value: number) => {
    const next = Math.max(0, Math.min(LOBBY_MUSIC_MAX_VOLUME, value));
    setLobbyMusicVolumeState(next);
    applyLobbyMusicVolume(next);
    AsyncStorage.setItem("mahalle:lobbyMusicVolume", String(next));
  }, []);

  const toggleDiscussionMusic = useCallback(() => {
    setDiscussionMusicEnabledState((prev) => {
      const next = !prev;
      AsyncStorage.setItem("mahalle:discussionMusicEnabled", String(next));
      if (isMidGame()) {
        setSystemToast({
          message: next ? "Tartışma/Oylama Müziği açıldı" : "Tartışma/Oylama Müziği kapatıldı",
          id: Date.now(),
          icon: next ? "music" : "volume-x",
          alwaysShow: true,
        });
  ...
  const setDiscussionMusicVolume = useCallback((value: number) => {
    const next = Math.max(0, Math.min(DISCUSSION_MUSIC_MAX_VOLUME, value));
    setDiscussionMusicVolumeState(next);
    applyDiscussionMusicVolume(next);
    AsyncStorage.setItem("mahalle:discussionMusicVolume", String(next));
  }, []);
