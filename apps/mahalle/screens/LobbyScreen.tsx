...
export default function LobbyScreen() {
  ...
  const [showQRJoinScanner, setShowQRJoinScanner] = useState(false);
  ...
  function parseScannedRoomCode(rawData: string): string | null {
    const raw = String(rawData ?? "").trim();
    if (!raw) return null;

    const parsed = Linking.parse(raw);
    if (parsed.queryParams?.code) {
      const fromQuery = String(parsed.queryParams.code).toUpperCase().trim();
      if (fromQuery.length >= 3) return fromQuery;
    }

    const fallback = raw.toUpperCase().replace(/[^A-Z0-9]/g, "");
    if (fallback.length >= 3) return fallback.slice(0, 10);
    return null;
  }

  async function attemptJoinWithCode(nextCode?: string) {
    if (!(myNickname ?? "").trim()) {
      showError("Önce mahalleli adını girmelisin.");
      return;
    }
    ...
    setBusy(true);
    haptic(Haptics.ImpactFeedbackStyle.Medium);
    const res = await joinRoom(roomCode, (myNickname ?? "").trim());
    setBusy(false);
    if (!res.ok) showError(res.error ?? "Oda bulunamadı. Kodu kontrol et.");
  }

  async function handleJoin() {
    await attemptJoinWithCode();
  }

  async function handleOpenScanner() {
    if (Platform.OS === "web") {
      showError("QR tarama bu cihazda desteklenmiyor.");
      return;
    }
    if (!cameraPermission?.granted) {
      const res = await requestCameraPermission();
      if (!res.granted) {
        showError("QR okutmak için kamera izni gerekli.");
        return;
      }
    }
    setScannerLocked(false);
    setShowQRJoinScanner(true);
  }
  ...
  if (!inRoom) {
    return (
      <>
            ...
            >
              ...
              {mode === "join" ? (
                <>
                  ...
                  >
                    <Feather name="camera" size={16} color="#C3AEFF" />
                    <Text style={styles.qrJoinBtnText}>QR ile Katıl</Text>
                  </Pressable>
                </>
              ) : null}
        ...
        <Modal
          visible={showQRJoinScanner}
          animationType="slide"
          transparent={false}
          onRequestClose={() => setShowQRJoinScanner(false)}
        >
          <View style={styles.scannerRoot}>
            <View style={[styles.scannerTopBar, { paddingTop: insets.top + 8 }]}>
              <Text style={styles.scannerTitle}>QR ile Odaya Katıl</Text>
              <Pressable onPress={() => setShowQRJoinScanner(false)} hitSlop={8} style={styles.scannerCloseBtn}>
                <Feather name="x" size={20} color="#E8DEFF" />
              </Pressable>
            </View>
            <View style={styles.scannerBody}>
              <View style={styles.scannerCameraWrap}>
                <CameraView
                  ...
                  onBarcodeScanned={scannerLocked ? undefined : async ({ data }) => {
                    const parsedCode = parseScannedRoomCode(String(data ?? ""));
                    if (!parsedCode) {
                      showError("Geçerli bir oda QR kodu okunamadı.");
                      return;
                    }
                    setScannerLocked(true);
                    setCode(parsedCode);
                    setMode("join");
                    setShowQRJoinScanner(false);
                    await attemptJoinWithCode(parsedCode);
                  }}
                />
              ...
              <Text style={styles.scannerHint}>Oda QR kodunu çerçevenin içine hizala.</Text>
            </View>
          </View>
        </Modal>
      </>
  ...
  return (
    <>
    <ScrollView
      contentContainerStyle={[styles.scroll, { paddingTop: 8, paddingBottom: insets.bottom + 24 }]}
      showsVerticalScrollIndicator={false}
    >
      {/* ── SECTION 1: Oda Kodu ── */}
...
function HostSettings({ state, emit }: any) {
  ...
  const joinedCount = (state.players as any[]).length;
  const previewPlayerCount = Math.max(joinedCount, 4);
                  ...
                  {isExpanded && (
                    <View style={styles.packageDetailsBox}>
                      ...
                      <Text style={styles.distPreviewTitle}>
                        Odaya katılan {joinedCount} kişi (hesaplanan: {previewPlayerCount} oyuncu) için olası taraf dağılımı:
                      </Text>
...
const styles = StyleSheet.create({
  ...
  scannerOverlay: {
    ...
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "rgba(7, 4, 16, 0.22)",
  },
  scannerFrame: {
    width: 240,
    height: 240,
