...
import {
  retryMusicAfterUserGesture,
  setDiscussionMusicEnabled,
  setDiscussionMusicVolume,
  setLobbyMusicEnabled,
  setLobbyMusicVolume,
  unloadAllMusic,
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const inDiscussionFlow =
      !!state && (state.phase === "DAY" || state.phase === "VOTE" || state.phase === "VOTE_RUNOFF");
    const shouldPlayLobbyMusic = lobbyMusicEnabled && inLobbyFlow;
    const shouldPlayDiscussionMusic = discussionMusicEnabled && inDiscussionFlow;
    let cancelled = false;
    (async () => {
      await Promise.all([
        setLobbyMusicVolume(lobbyMusicVolume),
        setDiscussionMusicVolume(discussionMusicVolume),
      ]);
      if (!cancelled) {
        await Promise.all([
          setLobbyMusicEnabled(shouldPlayLobbyMusic),
          setDiscussionMusicEnabled(shouldPlayDiscussionMusic),
        ]);
      }
    })();
    return () => {
  ...
  useEffect(() => {
    ...
    if (!lobbyMusicEnabled && !discussionMusicEnabled) return;
    const unlock = () => {
      retryMusicAfterUserGesture();
    };
  ...
  }, [lobbyMusicEnabled, discussionMusicEnabled]);
